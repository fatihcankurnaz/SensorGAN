...
def train(dataloader, config, device):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.MAX_EPOCH):
        for current_batch, data in enumerate(dataloader, 0):
            ...
            lidar_gen_error = lidar_gen_error_disc+ config.LIDAR_GENERATOR.PIXEL_LAMBDA* lidar_gen_error_pixel
            ...
            if current_batch % 5 == 0:
                print(
                    '[%d/%d][%d/%d]\t\t Camera to Lidar GAN Loss_D R/F: %.4f/%.4f \t Tot = %.4f  \t '
                    'Loss_G: %.4f \t PixelError: %.4f\t D(x): %.4f \t D(G(z)): %.4f '
                    % (epoch, config.TRAIN.MAX_EPOCH, current_batch, len(dataloader),
                       ...
                       lidar_disc_real_output.mean().item(), lidar_disc_fake_output.mean().item()))
